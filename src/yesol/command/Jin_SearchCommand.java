...
public class Jin_SearchCommand implements Command{
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		...
		url = request.getRequestURL().toString() + "?catagory=" + catagory + "&search=" + searchCate + "&word=" + word + "&page=";
		
		// total 만들기 
		// 1. 쿼리문 수행
		totalPage = dao.selectSearchTotalBoard(pageRows, catagory, word, searchCate);
		...
		try {
			...
			dao = new WriteDAO();
			arr = dao.search(catagory, word, searchCate, curPage, pageRows);
